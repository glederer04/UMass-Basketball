...
opp_sg <- 'Rahsool Diggins'
opp_sf <- 'Jayden Ndjigue'
opp_pf <- 'Matt Cross'
opp_C <- 'Josh Cohen'
UMass_pg <- 'Keon Thompson'
UMass_sg <- 'Rahsool Diggins'
UMass_sf <- 'Jayden Ndjigue'
UMass_pf <- 'Matt Cross'
UMass_C <- 'Josh Cohen'
player_names <- c('KEON.THOMPSON', 'RAHSOOL.DIGGINS', 'JAYDEN.NDJIGUE', 'MATT.CROSS', 'JOSH.COHEN')
...
#Player Comparison (good)
```{r}
opp_roster <- ncaahoopR::get_roster(ESPN_team, season = '2023-24')
...
UMass_roster <- ncaahoopR::get_roster('UMass', season = '2023-24')
...
```{r}
team_stats <- bart_team_box(year=2023)
teams <- bart_teams(year=2023, conf = NULL)
...
#Player Stats
```{r}
team_roster <- ncaahoopR::get_roster(ESPN_team, season = '2023-24')
team_roster <- team_roster %>%
  mutate(num = number) %>%
  select(num, name, position, height, player_image, class, weight)

bart_player_stats <- bart_player_season(year=2024, stat = 'all')
...
#lineups
```{r}
opp_schedule <- get_team_schedule(season = '2023-24', team.name = NCAA_team)
